      ...
      <header className="w-full bg-white shadow-sm px-6 py-3 flex justify-between items-center relative z-50">
        ...
        <div className="flex items-center gap-6">
          <NavLink to="/" className="w-28">
            <img src="/images/funval-logo.svg" alt="Funval logo" className="object-contain w-full" />
          </NavLink>
          ...
          <nav className="hidden md:flex gap-6 text-gray-700">
            {navLinks.map((link) => (
              <NavLink
                key={link.name}
                to={link.path}
                className={({ isActive }) => `hover:text-blue-600 ${isActive ? "font-semibold text-black" : ""}`}
              >
                {link.name}
              </NavLink>
            ))}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-1">
              <span className="text-sm font-semibold bg-gray-100 rounded-full px-3 py-1 hover:bg-gray-200">
                {`${user.f_name[0]}${user.f_lastname[0]}`}
              </span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
          ...
          <button className="md:hidden flex flex-col justify-center items-center w-8 h-8" onClick={() => setMenuOpen(!menuOpen)}>
            <span className={`block w-6 h-0.5 bg-gray-700 transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-1.5" : ""}`}></span>
            <span className={`block w-6 h-0.5 bg-gray-700 my-1 transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`}></span>
            <span className={`block w-6 h-0.5 bg-gray-700 transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-1.5" : ""}`}></span>
          </button>
        </div>
